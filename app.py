...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    def _index_text(text: str, source_name: str):
        ...
        start_idx = len(st.session_state.all_chunks)
        for i, chunk in enumerate(chunks):
            st.session_state.all_chunks.append(chunk)
            st.session_state.all_embeddings.append(embeddings[i])
            st.session_state.all_sources.append(source_name)
            st.session_state.all_chunk_ids.append(f"{source_name}_{i}")

        all_emb = np.array(st.session_state.all_embeddings).astype(np.float32)
        labels, kmeans = cluster_chunks(all_emb, n_clusters=4)
        ...
        for cluster_id in set(labels):
            cluster_chunks_list = [
                st.session_state.all_chunks[i] for i in range(len(labels)) if labels[i] == cluster_id
            ]
            time.sleep(2)
        ...
        store_chunks(
            ...
            np.array(st.session_state.all_embeddings),
            "global",
            labels,
            ids=st.session_state.all_chunk_ids,
            sources=st.session_state.all_sources,
        )

        st.session_state.total_chunks_stored = len(st.session_state.all_chunks)
        ...
        for i in range(start_idx, len(labels)):
            tname = topic_names.get(int(labels[i]), f"Cluster {labels[i]}")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="cb-features">
      ...
      <div class="cb-feature"><div class="cb-feature-icon" style="background:#0d1e35">🏷️</div><h3>Global Topic Clustering</h3><p><strong style="color:#aaa">K-Means</strong> re-clusters <strong style="color:#aaa">all chunks across every uploaded document</strong> on each new upload, keeping cluster IDs consistent in ChromaDB so old documents are never mis-routed to a stale topic.</p></div>
      ...
      <div class="cb-feature"><div class="cb-feature-icon" style="background:#1e2a1a">📋</div><h3>Copy Answer</h3><p>Every AI answer has a one-click <strong style="color:#aaa">copy button</strong>, so users can paste responses straight into emails, docs, or tickets.</p></div>
      <div class="cb-feature"><div class="cb-feature-icon" style="background:#2a1a1e">📊</div><h3>Inline Confidence Bar</h3><p>A color-coded confidence bar (<strong style="color:#4CAF7D">green</strong>/<strong style="color:#E5945B">orange</strong>/<strong style="color:#E57070">red</strong>) is shown directly under every answer — no need to open an expander to gauge trust.</p></div>
      <div class="cb-feature"><div class="cb-feature-icon" style="background:#1a1e2a">📚</div><h3>Source Citation Panel</h3><p>Each answer links back to the exact document chunks used to generate it — with source name and a text snippet — so users can verify claims at a glance.</p></div>
      <div class="cb-feature"><div class="cb-feature-icon" style="background:#2a261a">👍</div><h3>Answer Feedback</h3><p>Users can rate any answer 👍 or 👎 directly in the chat, building a lightweight signal for which responses are actually useful.</p></div>
      <div class="cb-feature"><div class="cb-feature-icon" style="background:#1a2a26">📝</div><h3>Document Summarizer</h3><p>Right after indexing, Groq generates a 4–6 sentence summary of each document — visible in an expander on the Upload page, no need to ask a question just to get the gist.</p></div>
    </div>
    """, unsafe_allow_html=True)
