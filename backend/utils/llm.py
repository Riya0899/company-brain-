...
def generate_answer_with_retry(context, question, max_retries=3):
    ...
    while attempt < max_retries:
        ...
        if attempt == 1:
            extra_instruction = "Give a clear, professional answer."
        elif attempt == 2:
...
{extra_instruction}"""
        ...
        # Strip the SOURCES_USED tag before judging, so the evaluator only
        # scores the actual answer content, not the citation metadata.
